...
class InMemoryAuditStore(
    ...
    def append_replay_stable(
        ...
        """Append with replay comparison excluding recorded_at."""
        if not isinstance(record, self._record_type):
            return AppendReceipt(
                result=AppendResult.SCHEMA_REJECTED,
                record_id=None,
        ...
        with self._lock:
            ...
            if existing is not None:
                candidate = replace(
                    record,
                    recorded_at=(
                        existing.record.recorded_at
                    ),
                )
                try:
                    candidate_canonical = _canonical_bytes(
                        candidate
                    )
                except Exception as error:
                    return AppendReceipt(
                        result=(
                            AppendResult.SCHEMA_REJECTED
                        ),
                        record_id=record_id,
                        error_code=type(error).__name__,
                    )
            ...
            try:
                integrity_hash = self._integrity_hasher(
                    integrity_material
                )
